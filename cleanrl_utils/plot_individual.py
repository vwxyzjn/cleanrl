...
parser.add_argument('--interested-exp-names', nargs='+', default=['ppo_ac_positive_reward-shift-800000--adaptation-1000000--positive_likelihood-1-', 'ppo_ac_positive_reward-shift-800000--adaptation-1000000--positive_likelihood-0-'],
                    help='the hyper parameters tuned')
...
parser.add_argument('--seed', type=int, default=6,
                    help='seed of color palette shuffle')
...
for env in set(all_df["gym_id"]):
    ...
    ax = sns.lineplot(data=data.groupby(["seed", "algo"]).apply(_smooth).loc[data['algo'].isin(interested_exp_names)], x="global_step", y=args.feature_of_interest, hue="algo", units="seed", estimator=None, palette=current_palette_dict,
                      alpha=0.2)
    sns.lineplot(data=data.groupby(["seed", "algo"]).apply(_smooth).loc[data['algo'].isin(interested_exp_names)], x="global_step", y=args.feature_of_interest, hue="algo", ci=None, palette=current_palette_dict,
                 linewidth=2.0)
    ax.set(xlabel=args.x_label, ylabel=args.y_label)
    
    handles, labels = ax.get_legend_handles_labels()
    legend = ax.legend(handles=handles[1:len(labels)//2], labels=labels[1:len(labels)//2], loc='upper center', bbox_to_anchor=(0.5, -0.20),
          fancybox=True,)
    for text in legend.get_texts():
        text.set_text(exp_convert_dict[text.get_text()])
