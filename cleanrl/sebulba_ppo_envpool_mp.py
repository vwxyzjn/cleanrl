...
def parse_args():
    ...
    parser.add_argument("--profile", type=lambda x: bool(strtobool(x)), default=True, nargs="?", const=True,
...
if __name__ == "__main__":
    ...
    # rollout_queue = queue.Queue(maxsize=3)
    # params_queue = queue.Queue(maxsize=3)
    ...
    #         ),
    #     ).start()
    rollout_queue = mp.Queue(maxsize=3)
    params_queue = mp.Queue(maxsize=3)
    params_queue.put(jax.device_put(agent_state.params, devices[0]))
    for i in range(1):
        mp.Process(
            target=rollout,
