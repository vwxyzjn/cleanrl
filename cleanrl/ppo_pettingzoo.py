...
if __name__ == "__main__":
    ...
    parser.add_argument("--n-minibatch", type=int, default=32, help="the number of mini batch")
...
if args.capture_video:
    envs = VecVideoRecorder(
        envs, f"videos/{experiment_name}", record_video_trigger=lambda x: x % 150000 == 0, video_length=400
    )
...
for update in range(1, num_updates + 1):
    ...
    for step in range(0, args.num_steps):
        ...
        next_obs, rs, ds, infos = envs.step(torch.clamp(action, envs.action_space.low.mean(), envs.action_space.high.mean()))
    ...
    for i_epoch_pi in range(args.update_epochs):
        ...
        if args.kle_rollback:
            if (
                b_logprobs[minibatch_ind] - agent.get_action(b_obs[minibatch_ind], b_actions[minibatch_ind])[1]
            ).mean() > args.target_kl:
