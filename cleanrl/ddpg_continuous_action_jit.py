...
class Agent(nn.Module):
    def __init__(self, state_dim, action_dim):
        super().__init__()
        self.qf1 = QNetwork(state_dim, action_dim)
        self.actor = Actor(state_dim, action_dim)

        self.target_qf1 = QNetwork(state_dim, action_dim)
    ...
    def critic_loss(self, next_observations: torch.Tensor, rewards: torch.Tensor, dones: torch.Tensor, observations: torch.Tensor, actions: torch.Tensor, max_action: float, gamma: float):
        ...
        # print(f"qf1_a_values.sum():{qf1_a_values.sum()}, next_q_value.sum(): {next_q_value.sum()}")
        return qf1_loss
...
if __name__ == "__main__":
    ...
    agent = torch.jit.script(Agent(state_dim, action_dim).to(device))
    q_optimizer = optim.Adam(agent.qf1.parameters(), lr=args.learning_rate)
    ...
    obs = envs.reset()
    actor_fn = torch.jit.trace(agent.actor, torch.Tensor(obs).to(device))
    data = None
    for global_step in range(args.total_timesteps):
        ...
        if global_step < args.learning_starts:
            ...
            actions = actor_fn(torch.Tensor(obs).to(device))
            actions = np.array(
                [
                    (
                        actions.tolist()[0]
        ...
        if global_step > args.learning_starts:
            ...
            if global_step % args.policy_frequency == 0:
                ...
                # for param, target_param in zip(agent.qf1.parameters(), agent.target_qf1.parameters()):
                #     target_param.data.copy_(args.tau * param.data + (1 - args.tau) * target_param.data)
