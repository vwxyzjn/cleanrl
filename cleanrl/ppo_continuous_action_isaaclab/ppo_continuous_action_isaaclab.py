...
# parse the arguments
# split command-line arguments into IsaacLab and CleanRL parts
argv = sys.argv[1:]
if "--" in argv:
    ...
    isaac_argv = argv
    cleanrl_argv = []
# parse the IsaacLab arguments
args_cli = parser.parse_args(isaac_argv)
...
class ExtractObsWrapper(gym.ObservationWrapper):
    def observation(self, obs):
        # print(f"[INFO]: Extracting observation from the environment: {obs.keys()}")
        return obs["policy"]


if __name__ == "__main__":
    args = tyro.cli(Args, args=cleanrl_argv)
    # override CleanRL env and env count from IsaacLab parser if provided
    if args_cli.task is not None:
        args.env_id = args_cli.task
    ...
    envs = gym.make(args.env_id, cfg=env_cfg)
