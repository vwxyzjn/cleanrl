...
if __name__ == "__main__":
    ...
    optimizer = optim.Adam(q_network.parameters(), lr=args.learning_rate, eps=1.5e-4) # epilson = 1.5e-4 taken from https://arxiv.org/pdf/1710.02298.pdf
    ...
    # Nature DQN evaluation with epsilon=0.05 and 30 games
    epsilon = 0.05
    envs = gym.vector.SyncVectorEnv([make_env(args.env_id, 0, 0, args.capture_video, run_name)])
    num_evaluated_games = 30
    ...
    while True:
        ...
        # TRY NOT TO MODIFY: CRUCIAL step easy to overlook
        obs = next_obs
        if len(episodic_returns) >= num_evaluated_games:
            break

    print(f"Average episodic return: {np.mean(episodic_returns)} ± {np.std(episodic_returns)}")
    writer.add_scalar("evaluation/episodic_return_mean", np.mean(episodic_returns), global_step)
    writer.add_scalar("evaluation/episodic_return_std", np.std(episodic_returns), global_step)
