...
class WandbCallback(BaseCallback):
    ...
    def __init__(self, project: str, config: dict, experiment_name: str = None, verbose: int = 0, save_freq: int = 1000, save_path="./models"):
        super(WandbCallback, self).__init__(verbose)
        self.save_freq = save_freq
        self.save_path = save_path
        # self.name_prefix = name_prefix
        self.experiment_name = experiment_name if experiment_name else f"{type(self.model).__name__}_{int(time.time())}"
        wandb.init(
            project=project,
            config=config,
