...
if __name__ == "__main__":
    ...
    rb = ReplayBuffer(
        args.buffer_size,
        {"observations": {"shape": envs.observation_space.shape}, "actions": {}, "rewards": {}, "dones": {}},
        next_of="observations",
        stack_compress="observations",
    )
    ...
    for global_step in range(args.total_timesteps):
        ...
        rb.add(observations=obs, next_observations=real_next_obs, actions=actions, rewards=rewards, dones=dones)

        # TRY NOT TO MODIFY: CRUCIAL step easy to overlook
        if d:
            rb.on_episode_end()  # quirks about cpprb memory compress
        obs = next_obs
        ...
        if global_step > args.learning_starts and global_step % args.train_frequency == 0:
            ...
            old_val = q_network.forward(data["observations"]).gather(1, data["actions"].long()).squeeze()
