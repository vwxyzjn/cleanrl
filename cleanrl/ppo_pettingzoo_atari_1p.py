...
def parse_args():
    ...
    parser.add_argument("--env-id", type=str, default="pong_v2",
    ...
    parser.add_argument("--learning-player-index", type=int, default=0,
        help="the index of the player to learn")
...
if __name__ == "__main__":
    ...
    learning_player_idxs = torch.arange(args.learning_player_index, args.num_envs, 2).to(device)
    ...
    for update in range(1, num_updates + 1):
        ...
        # flatten the batch
        b_obs = obs[:, learning_player_idxs].reshape((-1,) + envs.single_observation_space.shape)
        b_logprobs = logprobs[:, learning_player_idxs].reshape(-1)
        b_actions = actions[:, learning_player_idxs].reshape((-1,) + envs.single_action_space.shape)
        b_advantages = advantages[:, learning_player_idxs].reshape(-1)
        b_returns = returns[:, learning_player_idxs].reshape(-1)
        b_values = values[:, learning_player_idxs].reshape(-1)
