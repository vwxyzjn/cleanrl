...
if __name__ == "__main__":
    ...
    parser.add_argument("--gym-id", type=str, default="starpilot", help="the id of the gym environment")
    parser.add_argument("--learning-rate", type=float, default=5e-4, help="the learning rate of the optimizer")
    parser.add_argument("--seed", type=int, default=1, help="seed of the experiment")
    parser.add_argument("--total-timesteps", type=int, default=1e8, help="total timesteps of the experiments")
    parser.add_argument(
        "--torch-deterministic",
    ...
    parser.add_argument("--n-aux-minibatch", type=int, default=16, help="the number of mini batch in the auxiliary phase")
    parser.add_argument("--n-aux-grad-accum", type=int, default=10, help="the number of gradient accumulation in mini batch")

    parser.add_argument("--n-minibatch", type=int, default=8, help="the number of mini batch")
    parser.add_argument("--num-envs", type=int, default=64, help="the number of parallel game environment")
...
if args.capture_video:
    envs = VecVideoRecorder(
        envs, f"videos/{experiment_name}", record_video_trigger=lambda x: x % 1000000 == 0, video_length=600
    )
...
for phase in range(starting_phase, num_phases):
    for policy_update in range(1, args.n_iteration + 1):
        ...
        # PPG Storage:
        storage_slice = slice(
            args.num_steps * args.num_envs * (policy_update - 1), args.num_steps * args.num_envs * policy_update
        )
