...
if __name__ == "__main__":
    ...
    def step_env_wrappeed(episode_stats, handle, action):
        # print(type(action.astype(jnp.float64)))
        handle, (next_obs, reward, next_done, info) = step_env(handle, action.astype(jnp.float64))
        new_episode_return = episode_stats.episode_returns + reward
        new_episode_length = episode_stats.episode_lengths + 1
        episode_stats = episode_stats.replace(
            episode_returns=(new_episode_return) * (1 - next_done) * (1 - info["TimeLimit.truncated"]),
            episode_lengths=(new_episode_length) * (1 - next_done) * (1 - info["TimeLimit.truncated"]),
            # only update the `returned_episode_returns` if the episode is done
            returned_episode_returns=jnp.where(
                next_done + info["TimeLimit.truncated"], new_episode_return, episode_stats.returned_episode_returns
            ),
            returned_episode_lengths=jnp.where(
                next_done + info["TimeLimit.truncated"], new_episode_length, episode_stats.returned_episode_lengths
            ),
        )
