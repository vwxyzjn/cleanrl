...
if __name__ == "__main__":
    ...
    parser.add_argument('--kle-sigma', type=float, default=2.0,
                        help='if the current kl exceeds the running average of kl + kle_sigma* running sigma, then use kle')
...
breach_count = np.array([0.,0.,0.])
kl_rms = RunningMeanStd(shape=(1,))
past_kl = np.array([1.5 * 0.01]*100)
for i in range(50):
     kl_rms.update(np.array([args.target_kl]))
...
while global_step < args.total_timesteps:
    ...
    for i_epoch_pi in range(args.update_epochs):
        ...
        kl_rms.update(np.array([approx_kl.item()]))
        kl_moving_average = np.mean(past_kl)
        
        breach_count[0] += float(approx_kl.item() > args.target_kl)
        breach_count[1] += float(approx_kl.item() > kl_moving_average+np.std(past_kl)*args.kle_sigma)
        breach_count[2] += float(approx_kl.item() > kl_rms.mean+np.sqrt(kl_rms.var)*args.kle_sigma)
        # 

        past_kl = np.append(past_kl[1:], [approx_kl.item()])
        approx_kls.append(approx_kl.item())
        if args.kle_stop:
            if approx_kl > kl_moving_average+np.std(past_kl)*args.kle_sigma:
                break
        if args.kle_rollback:
            if (logprobs - pg.get_action(obs, actions)[1]).mean() > kl_moving_average+np.std(past_kl)*args.kle_sigma:
    ...
    print("breach_count", breach_count)
