...
while global_step < args.total_timesteps:
    ...
    for step in range(args.episode_length):
        ...
        # total_reward += reward
        # new_state = new_state
        # rb.put((state, action, reward, is_done, new_state))
        # state = new_state
        # if is_done:
        ...
        if global_step > args.learning_starts and global_step % args.train_frequency == 0:
            ...
            # old_val = q_network(states_v).gather(1, actions_v.unsqueeze(-1)).squeeze(-1)
            # target_max = target_network(next_states_v).max(1)[0]
            # target_max[done_mask] = 0.0
            # target_max = target_max.detach()

            # td_target = target_max * args.gamma + rewards_v

            # loss = loss_fn(old_val, td_target)
    ...
    print("%d: done, reward %.3f, eps %.2f" % (
        global_step, rewards.sum(), epsilon
    ))
    ...
    # if len(rb.buffer) < args.learning_starts:
    #     continue
