...
while global_step < args.total_timesteps:
    ...
    total_reward += reward
    new_state = new_state
    exp_buffer.put((state, action, reward, is_done, new_state))
    state = new_state
    if is_done:
    ...
    if len(exp_buffer.buffer) < args.learning_starts:
        continue
    ...
    nn.utils.clip_grad_norm_(list(net.parameters()), args.max_grad_norm)
    optimizer.step()
