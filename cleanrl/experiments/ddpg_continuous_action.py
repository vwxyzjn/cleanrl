...
class Actor(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        mu = torch.tanh(self.fc_mu(x))*min(env.action_space.high) # Multipled by 2 because the action space of the Pendulum-v0 is [-2,2]
        return mu
...
while global_step < args.total_timesteps:
    ...
    for step in range(args.episode_length):
        ...
        if random.random() < epsilon:
            ...
            actions[step] = action.tolist()[0] + noise_scale * np.random.randn(output_shape)
