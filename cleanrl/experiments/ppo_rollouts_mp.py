...
if __name__ == "__main__":
    ...
    parser.add_argument('--num-processes', type=int, default=8,
                        help="the number of processes to run environments")
    parser.add_argument('--num-steps', type=int, default=5,
                        help="the number of steps ran per environment")
...
envs = make_vec_envs(args.gym_id, args.seed, args.num_processes, args.gamma, "/tmp/gym/", device, True)
...
class Policy(nn.Module):
    ...
    def get_action(self, x):
        ...
        action = probs.sample()
        return action, probs.log_prob(action).sum(1, keepdim=True)

    def get_logproba(self, x, actions):
        action_mean, action_logstd = self.forward(x)
        action_std = torch.exp(action_logstd)
        dist = Normal(action_mean, action_std)
        return dist.log_prob(actions).sum(1, keepdim=True)
...
while global_step < args.total_timesteps:
    ...
    for _ in range(args.update_epochs):
        ...
        for sample in data_generator:
            ...
            ratio = torch.exp(newlogproba - old_action_log_probs_batch)
            surrogate1 = ratio * adv_targ
            surrogate2 = ratio.clamp(1 - args.clip_coef, 1 + args.clip_coef) * adv_targ
            policy_loss = -torch.mean(torch.min(surrogate1, surrogate2))
            newvalues = vf.forward(obs_batch)
            vf_loss = torch.mean((newvalues - return_batch).pow(2))
