...
def make_env(gym_id, seed, idx):
    def thunk():
        ...
        return env
    return thunk
envs = DummyVecEnv([make_env(args.gym_id, args.seed+i, i) for i in range(1)])
# if args.prod_mode:
#     envs = VecPyTorch(
...
class AsyncEnvs:
    ...
    def __init__(self, env_fns, num_rollout_workers, storage):
        ...
        self.rollout_task_queues = [FastQueue(100000) for i in range(num_rollout_workers)]
        self.stats_queue = FastQueue(100000)
        self.policy_request_queue = FastQueue(100000)
        ...
        for rollout_worker_idx in range(self.num_rollout_workers):
            mp.Process(target=self.start_rollout_worker, args=(rollout_worker_idx, env_fns)).start()
            ...
            with sw.timer('act'):
                ...
                with sw.timer('rollouts'):
                    for task in tasks:
                        ...
                        next_obs[env_idx], r, d, info = envs[env_idx].step(actions[step,env_idx])
                        if d:
...
for update in range(1, num_updates+1):
    ...
    with sw.timer('main'):
        while True:
            ...
            with sw.timer('policy_request_queue'):
                waiting_started = time.time()
                policy_requests = []
                num_policy_requests = 0
                while num_policy_requests < min_num_requests and time.time() - waiting_started < wait_for_min_requests:
                    if async_envs.policy_request_queue.qsize() != 0:
                        policy_requests.extend(async_envs.policy_request_queue.get_many(timeout=0.005))
                        num_policy_requests += 1
            
            if len(policy_requests) != 0:
                with sw.timer('prepare data'):
