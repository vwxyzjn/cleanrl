...
while global_step < args.total_timesteps:
    ...
    for _ in range(args.update_epochs):
        ...
        importance_sampling = torch.exp(newlogproba - torch.Tensor(logprobs[:step+1]).to(device))
        policy_loss = -torch.mean(importance_sampling.detach() * newlogproba * torch.Tensor(advantages)[:step+1].to(device))
