...
for update in range(1, num_updates+1):
    ...
    with torch.no_grad():
        ...
        args.upgo = True
        if args.upgo:
            next_values = torch.cat((values[1:], last_value), axis=0)
            next_dones = torch.cat((dones[1:], torch.ones_like(dones[-1:])), axis=0)
            lambdas = ((rewards + (1.0 - next_dones) * next_values) >= values).double()
            lambdas = torch.cat((lambdas[1:], torch.ones_like(lambdas[-1:])), axis=0)
            
            # https://github.com/deepmind/trfl/blob/2c07ac22512a16715cc759f0072be43a5d12ae45/trfl/sequence_ops.py#L215
            # next_values is applied twice apparently
            # next_values = torch.cat((next_values[1:], last_value), axis=0)
            sequence = rewards + (1.0 - next_dones) * next_values * (1 - lambdas)
            discount = (1.0 - next_dones) * lambdas
            next_discount = torch.cat((torch.ones_like(discount[:1]), discount), axis=0)
            # calculate using the regular return
            returns = torch.zeros_like(sequence).to(device)
            for t in reversed(range(args.num_steps)):
                if t == args.num_steps - 1:
                    nextnonterminal = 1.0 - next_done
                    next_return = last_value
                else:
                    nextnonterminal = next_discount[t+1]
                    next_return = returns[t+1]
                returns[t] = sequence[t] + args.gamma * nextnonterminal * next_return
