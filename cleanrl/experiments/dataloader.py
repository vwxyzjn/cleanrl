...
class PolicyWorkerDataset(IterableDataset):
    ...
    def __iter__(self):
        while True:
            try:
                policy_requests = self.policy_request_queue.get_many(timeout=0.005)
            except Empty:
                continue
            for item in policy_requests:
                for inner_item in item:
                    # print(inner_item)
                    yield (np.array(inner_item), np.array(obs[tuple(inner_item)]))
