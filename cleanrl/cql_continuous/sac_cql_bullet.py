...
# tmp override for debugs
args.offline_gym_id = "hopper-bullet-medium-v0"
...
## Additional helper to load custom dataset
def get_dataset(h5path, obs_shape, act_shape):
    ...
    assert data_dict['observations'].shape[1:] == obs_shape, \
            'Observation shape does not match env: %s vs %s' % (str(data_dict['observations'].shape[1:]), str(obs_shape))
    assert data_dict['actions'].shape[1:] == act_shape, \
                'Action shape does not match env: %s vs %s' % (str(data_dict['actions'].shape[1:]), str(act_shape))
...
class ExperienceReplayDataset(Dataset):
    def __init__(self):
        ...
        self.dataset = get_dataset("./hopper-buller-expert-v0.hdf5", input_shape, output_shape)
    def __len__(self):
...
data_loader = DataLoader(ExperienceReplayDataset(), batch_size=args.batch_size, num_workers=2)

train_batch = iter(data_loader).next()
print("3 data loader good, so frar so good")
...
for epoch in range(args.n_epochs):
    # TODO: Additional loop to handle when the dataset does not have enough batch to match epoch_length
    for k, train_batch in enumerate(data_loader):
        ...
        if args.min_q_version == 2:
            ...
            all_q1 = th.cat([q1_rand - random_density, q1_curr_actions - curr_log_pis.view(batch_size, args.num_actions).detach(), q1_next_actions - next_log_pis.view(batch_size, args.num_actions).detach()], 1)
            all_q2 = th.cat([q2_rand - random_density, q2_curr_actions - curr_log_pis.view(batch_size, args.num_actions).detach(), q2_next_actions - next_log_pis.view(batch_size, args.num_actions).detach()], 1)
        else:
    ...
    print("[E%04d|I%08d] -- PLoss: %.3f -- QLoss: %.3f -- CQLQloss: %.3f --  Train Mean Ret: %.3f" %
