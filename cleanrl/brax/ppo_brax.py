...
class NormalizedEnv(gym.core.Wrapper):
    ...
    def step(self, action):
        obs, rews, dones, infos = self.env.step(action)
        if not self.is_vector_env:
            obs, rews, dones = jnp.array([obs]), jnp.array([rews]), jnp.array([dones])
        self.ret = self.ret * self.gamma + rews
        obs = self._obfilt(obs)
        if self.ret_rms:
            self.ret_rms.update(self.ret)
            rews = jnp.clip(rews / jnp.sqrt(self.ret_rms.var + self.epsilon), -self.cliprew, self.cliprew)
        self.ret.at[dones.astype(bool)].set(0)
        if not self.is_vector_env:
            return obs[0], rews[0], dones[0], infos
        return obs, rews, dones, infos
    ...
    def _obfilt(self, obs):
        if self.ob_rms:
            self.ob_rms.update(obs)
            obs = jnp.clip((obs - self.ob_rms.mean) / jnp.sqrt(self.ob_rms.var + self.epsilon), -self.clipob, self.clipob)
            return obs
        else:
            return obs
