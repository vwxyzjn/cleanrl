...
entropys = torch.zeros((args.num_steps, args.num_envs)).to(device)
...
for update in range(1, num_updates+1):
    ...
    for step in range(0, args.num_steps):
        ...
        with torch.no_grad():
            action, logproba, entropy, vs = agent.get_action_and_value(next_obs)
            values[step] = vs.flatten()

        actions[step] = action
        logprobs[step] = logproba
        entropys[step] = entropy
    ...
    # add entropys to the rewards
    rewards += entropys * args.ent_coef
