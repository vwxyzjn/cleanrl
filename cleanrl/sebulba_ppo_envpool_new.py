...
def parse_args():
    ...
    parser.add_argument("--actor-device-ids", nargs="+", default=[],
        help="the device ids that actor workers will use")
    parser.add_argument("--learner-device-ids", nargs="+", default=[],
        help="the device ids that actor workers will use")
...
def rollout(
    ...
    key = jax.device_put(key, devices[i])
...
if __name__ == "__main__":
    ...
    multi_device_update = jax.pmap(
        ...
        axis_name="devices",
        devices=[devices[int(i)] for i in args.learner_device_ids],
    ...
    params_queue.put(agent_state.params)
    for i in args.actor_device_ids:
        threading.Thread(
            target=rollout,
            args=(
                int(i),
                ...
                params_queue,
                writer,
                [devices[int(i)] for i in args.learner_device_ids],
            ),
        ).start()
    ...
    while True:
        ...
        # multi_device_update(agent_state, b_obs, b_actions, b_logprobs, b_advantages, b_returns, b_inds, envs.single_action_space.n, key)
