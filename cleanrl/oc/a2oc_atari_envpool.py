...
def layer_init(layer, std: float = nn.init.calculate_gain('relu'), bias_const: float = 0.):
...
class Agent(nn.Module):
    ...
    def sample_option(self, option_on_arrival: Tensor, qs: Tensor, terminations: Tensor, current_epsilon: float = 0.1) -> Tensor:
        B = option_on_arrival.shape[0]
        # eps-greedy candidate option
        candidate_option = torch.where(torch.rand_like(option_on_arrival, dtype=torch.float32) < current_epsilon,
    ...
    def step(self, x: Tensor, option_on_arrival: Tensor, is_done: Tensor, current_epsilon: float = 0.1):
        x = self.features(x)
        ...
        term_w = torch.bernoulli(beta_w) + is_done  # Sample terminations
        option = self.sample_option(option_on_arrival, q, term_w, current_epsilon)
        log_pi_w = torch.log_softmax(batched_index(option, self.actor(x).reshape(-1, self.num_options, self.num_actions)), -1)
        a = torch.multinomial(log_pi_w.exp(), 1)
        lp_a_w = log_pi_w.gather(-1, a).squeeze(-1)
        return (term_w, option, a.squeeze(-1)), (beta_w, lp_a_w), q
        # pi_w = torch.log_softmax(self.actor(batched_index(option, x)).reshape(-1, self.num_options, self.num_actions), -1)
...
if __name__ == "__main__":
    ...
    options = torch.zeros((args.num_steps, args.num_envs), dtype=torch.int64, device=device)
    logprobs = torch.zeros((args.num_steps, args.num_envs), device=device)
    rewards = torch.zeros((args.num_steps, args.num_envs), device=device)
    dones = torch.zeros((args.num_steps, args.num_envs), device=device)
    values = torch.zeros((args.num_steps, args.num_envs, args.num_options), device=device)
    avg_returns = deque(maxlen=20)

    # TRY NOT TO MODIFY: start the game
    global_step = 0
    start_time = time.time()
    current_option = torch.zeros(args.num_envs, device=device, dtype=torch.int64)
    next_obs = torch.tensor(envs.reset(), device=device)
    next_done = torch.ones(args.num_envs, device=device)
    num_updates = args.total_timesteps // args.batch_size
    out = agent.step(next_obs, current_option, next_done)
